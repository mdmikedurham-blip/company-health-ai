...
const statusConfig: Record<HealthStatus, { label: string; barColor: string; badge: string }> = {
  healthy: {
    label: "Healthy",
...
export function HealthDimensionRow({
  ...
  const isClickable = dimensionId && explain;

  const content = (
    <>
      <span className="w-28 shrink-0 text-[13px] text-zinc-400 sm:w-32">{name}</span>
      ...
      <span className="w-7 shrink-0 text-right text-[13px] font-medium tabular-nums text-zinc-300">
        {score}
      </span>
      {trend && <TrendIndicator trend={trend} value={trendValue} />}
      <span className={`hidden w-16 shrink-0 rounded-full border px-2 py-0.5 text-center text-[10px] font-medium lg:inline-block ${config.badge}`}>
        {config.label}
      </span>
    </>
  );
